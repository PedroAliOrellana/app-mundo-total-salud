...
<script>
  ...
  export default {
    ...
    beforeMount: async function(){
      ...
      const resGet = await axios('http://161.35.140.26:3000/suscriptores/' + resAfi.data.cedula_auspiciador)
      let data = {
          cedula:"Código",
          nombre: "Master",
          apellido: "",
      ...
      if(resGet.status == 200){
        ...
        const resAbu = await axios('http://161.35.140.26:3000/suscriptores/' + resAfi.data.cedula_abuelo)
         if(resAbu.status == 200){
            resAbu.data.parentesco = "Abuelo"
            this.tableData.push(resAbu.data)        
            const resBisa = await axios('http://161.35.140.26:3000/suscriptores/' + resAfi.data.cedula_bisabuelo)
