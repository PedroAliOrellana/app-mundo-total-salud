...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      eliminarSolicitud: async function(solicitud){
        if(confirm("Confirma que desea eliminar la solicitud de "+ solicitud.nombre + " " + solicitud.apellido)){
         try {
             const resElim = await axios.put('http://161.35.140.26:3000/solicitudesAfiliacion/'+ solicitud.cedula,{estatus:3},{
                      headers:{
      ...
      aceptarSolicitud: async function(solicitud){
        if(confirm("Confirma la aprobación de la solicitud de "+ solicitud.nombre + " " + solicitud.apellido)){
          try {
            const resAus = await axios.get('http://161.35.140.26:3000/suscriptores/'+solicitud.cedula_auspiciador)
            let data = {
            ...
            const resPost = await axios.post('http://161.35.140.26:3000/suscriptores',data,{
                          headers:{
                              Authorization: 'Bearer '+ localStorage.getItem('token')
                          }
            })
            if(resPost.status == 200)
            {
              ...
              const resUser = await axios.post('http://161.35.140.26:3000/users',datos)
              if(resUser.status ==200){
                const resPass = await axios.get('http://161.35.140.26:3000/users/restaurar/'+ solicitud.correo)
                if(resPass.status == 200){
                  const resSolicitud = await axios.put('http://161.35.140.26:3000/solicitudesAfiliacion/'+ solicitud.cedula,{estatus:2},{
                          headers:{
                              Authorization: 'Bearer '+ localStorage.getItem('token')
                          }
                  })
    ...
    beforeMount: async function(){
      const resGet = await axios('http://161.35.140.26:3000/solicitudesAfiliacion')
